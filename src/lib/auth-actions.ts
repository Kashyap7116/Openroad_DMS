...
const adminUsersFilePath = path.join(process.cwd(), 'database', 'admin', 'users.json');
const profilePicturesDir = path.join(process.cwd(), 'public', 'uploads', 'profile_pictures');
 ...
 */
export async function updateUser(email: string, newName: string, newImageFile: File | null) {
  try {
    const fileContent = await fs.readFile(adminUsersFilePath, 'utf-8');
    ...
    await fs.writeFile(adminUsersFilePath, JSON.stringify(users, null, 2), 'utf-8');
