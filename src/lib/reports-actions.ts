...
import { getAllVehicles, getVehicle } from './vehicle-actions';
import { getEmployeeAdjustments, getFinanceSummary } from './finance-actions';
import { getEmployees, getRawAttendanceData } from './hr-actions';
...
export async function getMasterReportData(): Promise<{
  ...
  const allMaintenance = vehicles.flatMap(v => (v.maintenance_history || []).map(m => ({...m, vehicle: v.vehicle, license_plate: v.license_plate})));
  const totalMaintenanceCost = allMaintenance.reduce((sum, item) => sum + (item.total || 0), 0);
  
  const totalHRPayout = (await getEmployeeAdjustments()).filter(adj => ['Bonus', 'Addition', 'Employee Expense'].includes(adj.type))
    .reduce((sum, adj) => sum + adj.amount, 0) 
    + allEmployees.reduce((sum, emp) => sum + (emp.job_details.salary || 0), 0);
  ...
  const purchaseCost = vehicles.reduce((sum, v) => sum + (v.fullData?.grandTotal || 0), 0);
  ...
  const salesRecords = vehicles.filter(v => v.status === 'Sold');
  const allFinancialTx = vehicles.flatMap(v => (v.financial_history || []).map(tx => ({...tx, vehicle: v.vehicle})));
...
export async function getVehicleLifecycleReport(originalLicensePlate: string): Promise<any> {
    const vehicle = await getVehicle(originalLicensePlate);
    if (!vehicle) {
        return null;
    }
    ...
    const maintenanceCost = (vehicle.maintenance_history || []).reduce((sum, item) => sum + (item.total || 0), 0);
    
    const otherExpenses = (vehicle.financial_history || [])
        .filter((tx: any) => tx.type === 'expense' && tx.category !== 'Vehicle Purchase Cost')
        .reduce((sum: number, tx: any) => sum + tx.amount, 0);

    const salePrice = vehicle.sale_details?.sale_details.sale_price || 0;
    const otherIncome = (vehicle.financial_history || [])
        .filter((tx: any) => tx.type === 'income' && tx.category !== 'Customer Transfer')
        .reduce((sum: number, tx: any) => sum + tx.amount, 0);
    ...
    const totalExpenses = totalPurchaseCost + maintenanceCost + otherExpenses + salesCommission;
    ...
    if(vehicle.bonus_history) {
        vehicle.bonus_history = vehicle.bonus_history.map((b: any) => ({
            ...b,
            employee_name: employees.find(e => e.employee_id === b.employee_id)?.personal_info?.name || b.employee_id
        }));
    }
...
export async function getCustomReport({ type, from, to, vehicleIds }: GetCustomReportParams) {
    if (type === 'vehicleHistory') {
        ...
        const validVehicles = selectedVehicles.filter(v => v !== null) as VehicleRecord[];

        const purchaseData = validVehicles.map(v => ({
            'Purchase ID': v.id, 'Date': v.date, 'Original License Plate': v.id, 
            'Current License Plate': v.license_plate, 'Vehicle': v.vehicle,
            'Year': v.fullData.year, 'Color': v.fullData.color, 'VIN': v.fullData.vin,
            'Seller': v.seller, 'Payment Type': v.paymentType, 'Vehicle Price': v.fullData.vehiclePrice,
        ...
        const maintenanceData = validVehicles.flatMap(v => 
            (v.maintenance_history || []).map(m => ({
                'Vehicle': v.vehicle, 'License Plate': v.license_plate, ...m
            }))
        );

        const financeData = validVehicles.flatMap(v => 
            (v.financial_history || []).map(f => ({
                'Vehicle': v.vehicle, 'License Plate': v.license_plate, ...f
            }))
        );

        const salesData = validVehicles.filter(v => v.sale_details).map(v => ({
             'Sale ID': v.sale_details.sale_id, 'Date': v.sale_details.sale_details.sale_date,
             'Vehicle': v.vehicle, 'License Plate': v.license_plate, 'Buyer': v.sale_details.buyer.name,
        ...
        const licenseData = validVehicles.flatMap(v => 
             (v.licence_history || []).map((lh: any) => ({
                'Original Plate': v.id, 'New Plate': lh.newLicensePlate,
                'Issue Date': lh.issueDate, 'Authority': lh.issuingAuthority,
                'Fee': lh.price
            }))
        );
    ...
    if (type === 'vehicleDateRange') {
        ...
        return vehicles.filter(v => {
            ...
            const totalMaintenanceCost = (v.maintenance_history || []).reduce((sum, m) => sum + m.total, 0);
            const totalIncome = (v.financial_history || []).filter(f => f.type === 'income').reduce((sum, f) => sum + f.amount, 0);
            const totalExpense = (v.financial_history || []).filter(f => f.type === 'expense').reduce((sum, f) => sum + f.amount, 0);
            const licenceFees = (v.licence_history || []).reduce((sum, lh) => sum + (lh.price || 0), 0);
            
            return {
                'Purchase ID': v.id, 'Purchase Date': v.date, 'License Plate': v.license_plate,
                'Vehicle': v.vehicle, 'Year': v.fullData.year, 'Status': v.status,
                ...
                'Sale Price': v.sale_details?.sale_details.sale_price,
                'Sales Freelancer Commission': v.sale_details?.freelancer_commission?.commission,
                'License Plate Change Fee': licenceFees,
                'Total Income on Vehicle': totalIncome,
                'Total Expense on Vehicle': totalExpense + totalMaintenanceCost + v.fullData.grandTotal + licenceFees,
                'Profit/Loss': totalIncome - (totalExpense + totalMaintenanceCost + v.fullData.grandTotal + licenceFees)
            };
        });
    }
    ...
    if (type === 'employeeDateRange') {
        ...
        for (const emp of allEmployees) {
            ...
            if (joinDate > toDate || (emp.job_details.status === 'Left' && new Date(emp.job_details.left_date) < fromDate)) {
                continue;
            }

            const adjustmentsInRange = employeeAdjustments.filter((adj: Adjustment) => {
                if (!adj.date) return false;
                const adjDate = new Date(adj.date);
                return adj.employee_id === emp.employee_id && adjDate >= fromDate && adjDate <= toDate;
            });
            ...
            const totalDebit = adjustmentsInRange
                .filter((adj: Adjustment) => ['Deduction', 'Employee Expense', 'Advance'].includes(adj.type))
                .reduce((sum: number, adj: Adjustment) => sum + adj.amount, 0);
             ...
             for (let d = new Date(fromDate); d <= toDate; d.setMonth(d.getMonth() + 1)) {
                ...
                try {
                    ...
                    if(rawData && rawData.length > 0) {
                         const empRawRecords = rawData.filter(r => r.employee_id === emp.employee_id);
                    ...
                    console.error(`Could not get attendance for ${emp.employee_id} for ${year}-${month}`, e);
                }
             }

            
            employeeReports.push({
                "Employee ID": emp.employee_id, "Name": emp.personal_info.name,
                "Department": emp.job_details.department, "Position": emp.job_details.position,
                "Joining Date": emp.job_details.joining_date, "Base Salary": emp.job_details.salary,
                "Status": emp.job_details.status, "Contact": emp.personal_info.contact,
                "Total Credit (Bonus/Addition)": totalCredit, "Total Debit (Deduction/Advance)": totalDebit,
                "Period Present Days": present + late, "Period Absent Days": absent,
