...
export function CommissionForm({ employees, vehicles, onSubmit, onCancel }: CommissionFormProps) {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        if (selectedEmployees.length === 0) {
            toast({ title: "Validation Error", description: "Please select at least one employee.", variant: "destructive" });
            return;
        }
        if (!selectedVehicle) {
        ...
        if (!commissionAmount || commissionAmount <= 0) {
            toast({ title: "Validation Error", description: "Commission amount must be greater than 0.", variant: "destructive" });
            return;
        }
    ...
    return (
        <form onSubmit={handleSubmit} className="space-y-4 pt-4 pb-4">
             ...
             <div className="space-y-2">
                <Label htmlFor="commissionAmount">Commission Amount (per employee)</Label>
                <Input
                    id="commissionAmount"
                    type="number"
