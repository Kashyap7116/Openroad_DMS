...
import { getVehicleLifecycleReport, getCustomReport } from '@/lib/reports-actions';
...
import { VehicleSelector } from '../shared/vehicle-selector';
...
export function CustomReport({ vehicles, employees }: CustomReportProps) {
  ...
  const [selectedHistoryVehicles, setSelectedHistoryVehicles] = useState<string[]>([]);
  ...
  const handleGenerateAndDownload = async () => {
      let reportName = '';
      let reportKey: 'vehicleDateRange' | 'employeeDateRange' | 'vehicleHistory' = 'vehicleDateRange';
      ...
      if(reportType === 'vehicleHistory') {
        ...
        reportKey = reportType === 'vehicleDateRange' ? 'vehicleDateRange' : 'employeeDateRange';
        reportName = reportType === 'vehicleDateRange' ? 'Vehicle_Date_Range_Report' : 'Employee_Date_Range_Report';
      }
      ...
      try {
          ...
          if (reportType === 'vehicleHistory' && reportData) {
            Object.keys(reportData).forEach(sheetName => {
                if(Array.isArray((reportData as any)[sheetName]) && (reportData as any)[sheetName].length > 0) {
        ...
        case 'vehicleLifecycle':
            return (
                <div className="flex items-end gap-4">
                  ...
                  <Button onClick={handleGenerateLifecycleReport} disabled={!selectedLifecycleVehicle || isLoading}>
            ...
            return (
                 <div className="flex items-end gap-4">
                    <div className="flex-grow space-y-2">
                     ...
                     <Button onClick={handleGenerateAndDownload} disabled={selectedHistoryVehicles.length === 0 || isLoading}>
            ...
            return (
                 <div className="flex items-end gap-4">
                    <div className="flex-grow space-y-2">
                     ...
                     <Button onClick={handleGenerateAndDownload} disabled={!dateFrom || !dateTo || isLoading}>
                        {isLoading ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Download className="mr-2 h-4 w-4" />}
                        Generate & Download
                    </Button>
                 </div>
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>Generate specialized reports for vehicle lifecycles or data exports.</CardDescription>
      </CardHeader>
        ...
        <div className="p-4 border rounded-lg bg-muted/50 space-y-4">
            <div className="space-y-2">
                 ...
                 <Select value={reportType} onValueChange={setReportType}>
                    ...
                    <SelectContent>
                        <SelectItem value="vehicleLifecycle">Vehicle Lifecycle Analysis</SelectItem>
                        <SelectItem value="vehicleHistory">Vehicle History Export (Excel)</SelectItem>
                        <SelectItem value="vehicleDateRange">Vehicle Data by Date Range (Excel)</SelectItem>
                        <SelectItem value="employeeDateRange">Employee Data by Date Range (Excel)</SelectItem>
                    </SelectContent>
                 </Select>
            </div>
            {renderFilterSection()}
