...
import { ReviewItem, ReviewSection, renderFilePreview } from "./shared/review-step";
...
export function PurchaseDetailsDialog({ isOpen, onClose, record }: PurchaseDetailsDialogProps) {
          ...
          <DialogDescription>
            Viewing record for License Plate {record.license_plate}.
             <Badge variant="outline" className={cn("ml-2",
                record.status === 'Completed' ? 'text-green-700 border-green-300 bg-green-50' :
                record.status === 'Processing' ? 'text-yellow-700 border-yellow-300 bg-yellow-50' :
                record.status === 'Sold' ? 'text-blue-700 border-blue-300 bg-blue-50' : ''
            )}>{record.status}</Badge>
          </DialogDescription>
        </DialogHeader>
        <ScrollArea className="flex-grow pr-4">
             <div className="space-y-6 py-4">
                 <ReviewSection title="Vehicle Information">
                    ...
                    <ReviewItem label="Original License Plate" value={originalLicensePlate} />
                    {newLicensePlate && newLicensePlate !== originalLicensePlate && <ReviewItem label="Current License Plate" value={newLicensePlate} isBold/>}
                ...
                <ReviewSection title="Pricing">
                    ...
                    <ReviewItem label="Vehicle Price" value={`฿${(formData.vehiclePrice || 0).toLocaleString()}`} />
                    <ReviewItem label="Tax Fee" value={`฿${(formData.taxFee || 0).toLocaleString()}`} />
                    <ReviewItem label="Insurance Fee" value={`฿${(formData.insuranceFee || 0).toLocaleString()}`} />
                    <ReviewItem label="Delivery/Service Fee" value={`฿${(formData.deliveryFee || 0).toLocaleString()}`} />
                    {formData.paymentType === 'Financing Pay-off' && <ReviewItem label="Financing Payoff" value={`฿${(formData.financingPayoff || 0).toLocaleString()}`} />}
                    <ReviewItem label="Grand Total" value={`฿${(formData.grandTotal || 0).toLocaleString()}`} isBold />
                </ReviewSection>
                ...
                <ReviewSection title="Freelancer Commission">
                    ...
                    <ReviewItem label="Commission Value" value={`${formData.commissionType === 'Fixed' ? '฿' : ''}${(formData.commissionValue || 0).toLocaleString()}${formData.commissionType === 'Percentage' ? '%' : ''}`} />
                    <ReviewItem label="ID Proof" value={renderFilePreview(formData.freelancerIdProof, "freelancer_id")} />
                </ReviewSection>
                ...
                <ReviewSection title="Insurance & Tax">
                     ...
                     <ReviewItem label="Insurance Document" value={renderFilePreview(formData.insuranceDoc, "insurance_doc.pdf")} />
                     <ReviewItem label="Tax Document" value={renderFilePreview(formData.taxDoc, "tax_doc.pdf")} />
                </ReviewSection>
                ...
                <ReviewSection title="Seller Information">
                     ...
                     <ReviewItem label="Seller ID Card" value={renderFilePreview(formData.sellerIdCard, "seller_id")} />
                </ReviewSection>
                ...
                <ReviewSection title="Vehicle Images">
                    <div>
                        ...
                        {formData.mainImage ? (
                             <Image src={typeof formData.mainImage === 'string' ? formData.mainImage : formData.mainImage.preview} alt="Main Vehicle" width={150} height={100} className="rounded-md object-cover"/>
                        ) : <p className="text-sm text-muted-foreground">Not provided</p>}
                    </div>
                     <div>
                        <h4 className="font-medium text-sm mt-4 mb-2">Additional Images</h4>
                        <div className="flex flex-wrap gap-2">
                        {formData.additionalImages && formData.additionalImages.length > 0 ? formData.additionalImages.map((file: any, index: number) => (
                             <Image key={index} src={typeof file === 'string' ? file : file.preview} alt={`Additional ${index+1}`} width={100} height={75} className="rounded-md object-cover"/>
                ...
                {record.bonus_history && record.bonus_history.length > 0 && (
                    <ReviewSection title="Employee Bonuses">
                        {record.bonus_history.map((bonus: any, index: number) => (
                           <div key={index} className="md:col-span-2 grid grid-cols-1 md:grid-cols-3 gap-x-8 gap-y-2 p-2 border-b">
                             <ReviewItem label="Employee" value={bonus.employee_name || bonus.employee_id} />
                             <ReviewItem label="Date" value={bonus.date} />
                             <ReviewItem label="Amount" value={bonus.amount} isAmount />
                             <ReviewItem label="Remarks" value={bonus.remarks} className="md:col-span-3"/>
                           </div>
                        ))}
                    </ReviewSection>
                )}
                ...
                <ReviewSection title="Licence History">
                  {record.licence_history.map((licence: any, index: number) => (
                    <div key={index} className="md:col-span-2 grid grid-cols-1 md:grid-cols-3 gap-x-8 gap-y-3 p-2 border-b">
                      <ReviewItem label="New Plate" value={licence.newLicensePlate} isBold />
                      <ReviewItem label="Issue Date" value={licence.issueDate} />
                      <ReviewItem label="Authority" value={licence.issuingAuthority} />
                    </div>
                  ))}
                </ReviewSection>
              )}
