...
import { BonusForm } from "@/modules/vehicles/components/bonus-form";
import { MultiStepPurchaseForm } from "@/modules/vehicles/forms/multi-step-purchase-form";
import {
  deleteVehicle,
  deleteVehicleBonus,
  getAllVehicles,
...
export default function PurchasePage() {
  ...
  const [filteredBonusRecords, setFilteredBonusRecords] = useState<
    BonusRecord[]
  >([]);
  ...
  const [isBonusDeleteConfirmOpen, setIsBonusDeleteConfirmOpen] =
    useState(false);
  ...
  const [editingBonusRecord, setEditingBonusRecord] =
    ...
    const allBonuses = allAdjustments
      .filter(
        (adj: any) =>
          adj.type === "Bonus" &&
          (adj.visit_type ||
            (adj.remarks || "").includes("Vehicle visit bonus"))
      )
      .map((adj: any) => ({
        ...adj,
        employee_name:
          allEmps.find((e) => e.employee_id === adj.employee_id)?.personal_info
            ?.name || adj.employee_id,
      }))
      .sort(
        (a: any, b: any) =>
  ...
  useEffect(() => {
    ...
    if (filters.vehicle) {
      filtered = filtered.filter((v) =>
        v.vehicle.toLowerCase().includes(filters.vehicle.toLowerCase())
      );
    }
    if (filters.status) {
      filtered = filtered.filter(
        ...
        <div className="text-left">
          ...
          {result.analysis && (
            <div className="mt-2 text-xs bg-destructive/20 p-2 rounded-md">
              ...
              <p>
                <strong>Cause:</strong> {result.analysis.probableCause}
              </p>
              <p>
                <strong>Suggestion:</strong> {result.analysis.suggestedSolution}
              </p>
            </div>
          )}
        </div>
  ...
  const handleLicenceFormSubmit = async (data: LicenceFormData) => {
    ...
    const result = await saveVehicle(
      data.originalLicensePlate,
      { licence_details: data },
      true
    );
  ...
  const handleDeleteBonusClick = (bonus: BonusRecord) => {
    setBonusToDelete({
      bonusId: bonus.id,
      vehicleId:
        bonus.license_plate === "No Booking" ? null : bonus.license_plate,
    });
  ...
  const handleDeleteBonusConfirm = async () => {
    if (bonusToDelete) {
      setIsLoading(true);
      const result = await deleteVehicleBonus(
        bonusToDelete.bonusId,
        bonusToDelete.vehicleId
      );
      if (result.success) {
        toast({
          title: "Bonus Deleted",
  ...
  const dashboardStats = useMemo(() => {
    const totalInvestment = vehicles.reduce(
      (acc, v) => acc + (v.fullData?.grandTotal || 0),
      0
    );
    const vehiclesInStock = vehicles.filter((v) => v.status !== "Sold").length;
    const vehiclesReadyForSale = vehicles.filter(
  ...
  const licenceHistory = useMemo(() => {
    let history = vehicles.filter(
      (v) => v.licence_history && v.licence_history.length > 0
    );
    ...
    if (licenceFilter) {
      ...
      history = history.filter(
        (v) =>
          (v.id || "").toLowerCase().includes(searchTerm) ||
          (v.license_plate || "").toLowerCase().includes(searchTerm) ||
          (v.licence_history || []).some((lh: any) =>
            (lh.newLicensePlate || "").toLowerCase().includes(searchTerm)
          )
      );
    }
    return history;
    ...
    <>
      ...
      >
        <Dialog
          open={bonusFormOpen}
          onOpenChange={(isOpen) => {
            setBonusFormOpen(isOpen);
            if (!isOpen) setEditingBonusRecord(null);
          }}
        >
            ...
            <DialogHeader>
              <DialogTitle>
                <span className="lang-en">
                  {editingBonusRecord ? "Edit" : "Add"} Employee Bonus
                </span>
                <span className="lang-th">
                  {editingBonusRecord ? "แก้ไข" : "เพิ่ม"} โบนัสพนักงาน
                </span>
              </DialogTitle>
              <DialogDescription>
                <span className="lang-en">
                  {editingBonusRecord
                    ? "Update this bonus record."
                    : "Record a bonus for an employee related to a vehicle inspection or purchase."}
                </span>
                <span className="lang-th">
                  {editingBonusRecord
                    ? "อัปเดตบันทึกโบนัสนี้"
                    : "บันทึกโบนัสสำหรับพนักงานที่เกี่ยวข้องกับการตรวจสอบหรือซื้อยานพาหนะ"}
                </span>
              </DialogDescription>
            </DialogHeader>
            <BonusForm
        ...
        <Dialog open={licenceFormOpen} onOpenChange={setLicenceFormOpen}>
          ...
          <DialogContent className="max-w-2xl">
            <DialogHeader>
              ...
              <DialogDescription>
                <span className="lang-en">
                  Select a vehicle and provide the new licence details. This
                  will create a new licence history record.
                </span>
                <span className="lang-th">
                  เลือกยานพาหนะและให้รายละเอียดใบอนุญาตใหม่
                  การดำเนินการนี้จะสร้างบันทึกประวัติใบอนุญาตใหม่
                </span>
              </DialogDescription>
            </DialogHeader>
            <LicenceForm
        ...
        >
          ...
          <DialogContent className="max-w-4xl h-[90vh] flex flex-col">
            <DialogHeader>
              <DialogTitle>
                <span className="lang-en">
                  {editingRecord ? "Edit" : "Add New"} Vehicle Purchase
                </span>
                <span className="lang-th">
                  {editingRecord ? "แก้ไข" : "เพิ่มใหม่"} การซื้อยานพาหนะ
                </span>
              </DialogTitle>
              <DialogDescription>
                <span className="lang-en">
                  {editingRecord
                    ...
                    : "Follow the steps to add a new vehicle to your inventory."}
                </span>
                <span className="lang-th">
                  {editingRecord
                    ? "อัปเดตรายละเอียดสำหรับยานพาหนะนี้"
                    : "ทำตามขั้นตอนเพื่อเพิ่มยานพาหนะใหม่ในคลังของคุณ"}
                </span>
              </DialogDescription>
            </DialogHeader>
      ...
      <Tabs defaultValue="purchases">
        <TabsList className="mb-4">
          <TabsTrigger value="purchases">
            <span className="lang-en">Vehicle Purchases</span>
            <span className="lang-th">การซื้อยานพาหนะ</span>
          </TabsTrigger>
          <TabsTrigger value="bonuses">
            <span className="lang-en">Employee Bonuses</span>
            <span className="lang-th">โบนัสพนักงาน</span>
          </TabsTrigger>
          <TabsTrigger value="licence">
            <span className="lang-en">Licence History</span>
            <span className="lang-th">ประวัติใบอนุญาต</span>
          </TabsTrigger>
        </TabsList>
        <TabsContent value="purchases">
          <Card>
            <CardHeader>
              <CardTitle>
                <span className="lang-en">Vehicle Purchase Records</span>
                <span className="lang-th">บันทึกการซื้อยานพาหนะ</span>
              </CardTitle>
              <CardDescription>
                <span className="lang-en">
                  Track and manage all vehicle acquisitions
                </span>
                <span className="lang-th">
                  ติดตามและจัดการการซื้อยานพาหนะทั้งหมด
                </span>
              </CardDescription>
            </CardHeader>
            <CardContent>
              <div className="mb-6 p-4 bg-gray-50 dark:bg-gray-900/50 rounded-lg">
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-4 items-end">
                  <div className="space-y-2">
                    ...
                    >
                      <span className="lang-en">ID or License Plate:</span>
                      <span className="lang-th">รหัสหรือป้ายทะเบียน:</span>
                    </Label>
                    <Input
                      id="filter-id"
                  ...
                  <div className="space-y-2">
                    ...
                    >
                      ...
                      <span className="lang-th">สถานะ:</span>
                    </Label>
                    <Input
                      id="filter-status"
                  ...
                  <div className="flex items-end gap-2 lg:col-span-full lg:col-start-4 lg:justify-end">
                    ...
                    >
                      ...
                      <span className="lang-th">ล้าง</span>
                    </Button>
                    <Button
                      className="w-full lg:w-auto"
                    ...
                    >
                      ...
                      <span className="lang-en">Export</span>
                      <span className="lang-th">ส่งออก</span>
                    </Button>
                  </div>
                </div>
              </div>
                    ...
                    {isLoading ? (
                      ...
                      filteredVehicles.map((purchase) => {
                        const latestLicensePlate =
                          getLatestLicensePlate(purchase);
                        return (
                          <TableRow
                            key={`${purchase.id}-${purchase.license_plate}`}
                          >
                            ...
                            <TableCell>
                              {latestLicensePlate !== purchase.id
                                ? latestLicensePlate
                                : "-"}
                            </TableCell>
                            ...
                            <TableCell className="text-center space-x-1">
                              ...
                              <Button
                                ...
                                onClick={() => handleOpenPurchaseForm(purchase)}
                              >
                      ...
                      <TableRow key="no-results">
                        ...
                        >
                          <span className="lang-en">
                            No vehicles found. Click "Record Vehicle Purchase"
                            to add one.
                          </span>
                          <span className="lang-th">
                            ไม่พบยานพาหนะ คลิก "บันทึกการซื้อยานพาหนะ"
                            เพื่อเพิ่ม
                          </span>
                        </TableCell>
                      </TableRow>
        ...
        <TabsContent value="bonuses">
          <Card>
            <CardHeader>
              <CardTitle>
                <span className="lang-en">Employee Bonus Records</span>
                <span className="lang-th">บันทึกโบนัสพนักงาน</span>
              </CardTitle>
              <CardDescription>
                <span className="lang-en">
                  A log of all bonuses awarded to employees for vehicle-related
                  activities.
                </span>
                <span className="lang-th">
                  บันทึกโบนัสทั้งหมดที่มอบให้กับพนักงานสำหรับกิจกรรมที่เกี่ยวข้องกับยานพาหนะ
                </span>
              </CardDescription>
            </CardHeader>
            <CardContent>
              <div className="mb-6 p-4 bg-gray-50 dark:bg-gray-900/50 rounded-lg">
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 items-end">
                  <div className="space-y-2">
                    ...
                    <Input
                      id="filter-bonus-fromDate"
                      type="date"
                      value={bonusFilters.fromDate}
                      onChange={handleBonusFilterChange}
                      className="bg-white dark:bg-gray-900"
                    />
                  </div>
                  <div className="space-y-2">
                    <Label
                      htmlFor="filter-bonus-toDate"
                      className="text-xs font-medium text-gray-500"
                    >
                    ...
                    <Input
                      id="filter-bonus-toDate"
                      type="date"
                      value={bonusFilters.toDate}
                      onChange={handleBonusFilterChange}
                      className="bg-white dark:bg-gray-900"
                    />
                  </div>
                  <Button variant="outline" onClick={clearBonusFilters}>
              ...
              <div className="overflow-x-auto">
                <Table>
                  <TableHeader>
                    <TableRow>
                      <TableHead>
                        <span className="lang-en">Date</span>
                        <span className="lang-th">วันที่</span>
                      </TableHead>
                      <TableHead>
                        <span className="lang-en">Employee</span>
                        <span className="lang-th">พนักงาน</span>
                      </TableHead>
                      <TableHead>
                      ...
                      <TableHead>
                        <span className="lang-en">Remarks</span>
                        <span className="lang-th">หมายเหตุ</span>
                      </TableHead>
                      <TableHead className="text-center">
                        <span className="lang-en">Actions</span>
                        ...
                        <TableRow key={bonus.id}>
                          ...
                          <TableCell className="text-center space-x-1">
                            ...
                            <Button
                              ...
                              size="icon"
                              className="h-8 w-8 text-gray-400 hover:text-red-600"
                              onClick={() => handleDeleteBonusClick(bonus)}
                            >
                              <Trash2 className="h-4 w-4" />
                            </Button>
                          </TableCell>
        ...
        <TabsContent value="licence">
          <Card>
            <CardHeader>
              <CardTitle>
                <span className="lang-en">Licence Change History</span>
                <span className="lang-th">ประวัติการเปลี่ยนใบอนุญาต</span>
              </CardTitle>
              <CardDescription>
                <span className="lang-en">
                  A log of all vehicles that have had their licence plates
                  changed.
                </span>
                <span className="lang-th">
                  บันทึกยานพาหนะทั้งหมดที่มีการเปลี่ยนป้ายทะเบียน
                </span>
              </CardDescription>
            </CardHeader>
            <CardContent>
              ...
              <div className="overflow-x-auto">
                <Table>
                  <TableHeader>
                    <TableRow>
                      <TableHead>
                        <span className="lang-en">New Licence Plate</span>
                        <span className="lang-th">ป้ายทะเบียนใหม่</span>
                      </TableHead>
                      <TableHead>
                        <span className="lang-en">Previous Licence Plate</span>
                        <span className="lang-th">ป้ายทะเบียนเก่า</span>
                      </TableHead>
                      <TableHead>
                        <span className="lang-en">Vehicle</span>
                        <span className="lang-th">ยานพาหนะ</span>
                      ...
                      <TableHead>
                        <span className="lang-en">Issue Date</span>
                        <span className="lang-th">วันที่ออก</span>
                      </TableHead>
                      <TableHead>
                        <span className="lang-en">Issuing Authority</span>
                        <span className="lang-th">ผู้ออกใบอนุญาต</span>
                      </TableHead>
                    </TableRow>
                  </TableHeader>
                  <TableBody>
                    {isLoading ? (
                      ...
                      licenceHistory.map((v) =>
                        (v.licence_history || []).map(
                          (lh: any, index: number) => {
                            const vehicleRecord = vehicles.find(
                              (vec) => vec.id === lh.originalLicensePlate
                            );
                            return (
                      ...
                      <TableRow>
                        ...
                        >
                          <span className="lang-en">
                            No licence change records found.
                          </span>
                          <span className="lang-th">
                            ไม่พบบันทึกการเปลี่ยนแปลงใบอนุญาต
                          </span>
                        </TableCell>
                      </TableRow>
                    )}
          ...
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              <span className="lang-en">
                ...
                vehicle record file and all associated data.
              </span>
              <span className="lang-th">
                การกระทำนี้ไม่สามารถยกเลิกได้
                การดำเนินการนี้จะลบไฟล์บันทึกยานพาหนะและข้อมูลที่เกี่ยวข้องทั้งหมดอย่างถาวร
              </span>
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            ...
            >
              ...
              <span className="lang-th">ลบ</span>
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
      ...
      <AlertDialog
        open={isBonusDeleteConfirmOpen}
        onOpenChange={setIsBonusDeleteConfirmOpen}
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>
            ...
            <AlertDialogDescription>
              <span className="lang-en">
                ...
                bonus record from both the vehicle and the employee's payroll
                data.
              </span>
              <span className="lang-th">
                การกระทำนี้ไม่สามารถยกเลิกได้
                การดำเนินการนี้จะลบบันทึกโบนัสนี้อย่างถาวรทั้งจากข้อมูลยานพาหนะและข้อมูลเงินเดือนของพนักงาน
              </span>
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel onClick={() => setBonusToDelete(null)}>
              <span className="lang-en">Cancel</span>
              <span className="lang-th">ยกเลิก</span>
            </AlertDialogCancel>
            <AlertDialogAction
              onClick={handleDeleteBonusConfirm}
              className="bg-destructive hover:bg-destructive/90"
            >
              <span className="lang-en">Delete Bonus</span>
              <span className="lang-th">ลบโบนัส</span>
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
