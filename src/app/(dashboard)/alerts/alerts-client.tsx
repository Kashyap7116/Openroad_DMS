...
import {
  contractIssueAlerts,
  ContractIssueAlertsInput,
  ContractIssueAlertsOutput,
} from "@/ai/flows/contract-issue-alerts";
import { generateDocumentExpiryAlerts } from "@/ai/flows/document-expiry-alerts";
import {
  errorAnalysisAlerts,
  ErrorAnalysisAlertsInput,
  ErrorAnalysisAlertsOutput,
} from "@/ai/flows/error-analysis-alerts";
...
import {
  getErrorLogs,
  saveErrorLog,
} from "@/modules/shared/services/alerts-actions";
  ...
  useEffect(() => {
    ...
    if (filters.docType) {
      filtered = filtered.filter(
        (alert) => alert.documentType === filters.docType
      );
    }
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          AI-powered alerts for documents nearing their expiration date.
          Checking {documentCount} documents.
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 items-end">
            <div className="space-y-2">
              ...
              <Select
                ...
                onValueChange={(value) => handleFilterChange("docType", value)}
              >
                <SelectTrigger>
        ...
        {isLoading ? (
          ...
          <Alert>
            ...
            <AlertDescription>
              No documents match your filter criteria or none are nearing expiry
              within the {alertThresholdDays}-day threshold.
            </AlertDescription>
          </Alert>
        )}
      </CardContent>
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Paste a contract below to have it analyzed for potential issues by
            our AI.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="space-y-2">
            ...
            <Textarea
              ...
              onChange={(e) =>
                setFormState({ ...formState, contractText: e.target.value })
              }
            />
          </div>
            ...
            <div className="space-y-2">
              ...
              <Select
                ...
                onValueChange={(value: "sales" | "purchase") =>
                  setFormState({ ...formState, contractType: value })
                }
              >
        ...
        <CardFooter>
          <Button
            ...
            disabled={isLoading || !formState.contractText}
          >
            {isLoading ? (
              <>
        ...
        <CardContent className="flex-grow">
          ...
          {result && (
            <div className="space-y-4">
              <div>
                <Badge
                  variant={result.hasIssues ? "destructive" : "default"}
                  className={
                    !result.hasIssues ? "bg-accent hover:bg-accent/90" : ""
                  }
                >
...
type ErrorLog = ErrorAnalysisAlertsOutput & {
  log_id: string;
  errorText: string;
  context?: string;
  created_at: string;
...
function ErrorLogAnalysis() {
  const [formState, setFormState] = useState<ErrorAnalysisAlertsInput>({
    errorText: "",
    context: "",
  });
    ...
    <div className="space-y-6">
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Paste a raw error message or stack trace to get an AI-generated
        ...
        <CardContent className="space-y-4">
          <div className="space-y-2">
            ...
            <Textarea
              ...
              onChange={(e) =>
                setFormState({ ...formState, errorText: e.target.value })
              }
            />
          </div>
          <div className="space-y-2">
        ...
        <CardFooter className="justify-between">
          <Button
            onClick={handleAnalyze}
            disabled={isLoading || !formState.errorText}
          >
            {isLoading ? (
              <>
              ...
              <>
                <BrainCircuit className="mr-2 h-4 w-4" /> Analyze Error
              </>
            )}
          </Button>
          {result && (
            <div className="flex items-center gap-2">
              ...
              <Badge className={cn(severityStyles[result.severity])}>
                {result.severity}
              </Badge>
            </div>
          )}
          ...
          <CardContent className="space-y-4">
            ...
            <div>
              ...
              <pre className="text-sm bg-muted p-3 rounded-md overflow-x-auto">
                <code>{result.suggestedSolution}</code>
              </pre>
            </div>
          </CardContent>
        </Card>
              ...
              {isLogLoading ? (
                ...
                errorLogs.map((log) => (
                  <TableRow key={log.log_id}>
                    ...
                    <TableCell>
                      <Badge className={cn(severityStyles[log.severity])}>
                        {log.severity}
                      </Badge>
                    </TableCell>
  ...
  return (
    <Tabs defaultValue="expiry-alerts" className="space-y-4">
      <TabsList className="grid w-full grid-cols-3 md:w-[600px]">
        <TabsTrigger value="expiry-alerts">
          <AlertTriangle className="mr-2 h-4 w-4" /> Document Expiry
        </TabsTrigger>
        <TabsTrigger value="contract-analysis">
          <FileText className="mr-2 h-4 w-4" /> Contract Analysis
        </TabsTrigger>
        <TabsTrigger value="error-analysis">
