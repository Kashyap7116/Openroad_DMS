...
import { EmployeeDetailsDialog } from "@/modules/hr/components/employee-details-dialog";
import { EmployeeForm } from "@/modules/hr/forms/employee-form";
import {
  deleteEmployee,
  getCountries,
  getEmployees,
  saveEmployee,
} from "@/modules/hr/services/hr-actions";
...
export default function EmployeesPage() {
  ...
  const handleFormSubmit = async (data: EmployeeRecord) => {
    ...
    try {
      // Use the proper Supabase function for saving
      const result = await saveEmployee(
        editingRecord?.employee_id || null,
        data
      );
      ...
      if (result.success) {
        ...
        toast({
          ...
          description: `Record for ${data.personal_info.name} (ID: ${
            "employee_id" in result ? result.employee_id : "N/A"
        ...
        if ("analysis" in result && result.analysis) {
          console.error("Error analysis:", result.analysis);
        }
      }
    } catch (error) {
      ...
      toast({
        title: "Error",
        description: "Failed to save employee record. Please try again.",
        variant: "destructive",
      });
    }
  ...
  return (
    <>
            ...
            <DialogHeader>
              <DialogTitle>
                <span className="lang-en">
                  {editingRecord ? "Edit" : "Add"} Employee Record
                </span>
                <span className="lang-th">
                  {editingRecord ? "แก้ไข" : "เพิ่ม"} บันทึกพนักงาน
                </span>
              </DialogTitle>
              <DialogDescription>
                <span className="lang-en">
                ...
                <span className="lang-th">
                  {editingRecord
                    ? "อัปเดตรายละเอียดสำหรับพนักงานนี้"
                    : "กรอกแบบฟอร์มเพื่อเพิ่มพนักงานใหม่ในระบบ"}
                </span>
              </DialogDescription>
            </DialogHeader>
      ...
      <Card>
        <CardHeader>
          <CardTitle>
            <span className="lang-en">Employee List</span>
            <span className="lang-th">รายชื่อพนักงาน</span>
          </CardTitle>
          <CardDescription>
            <span className="lang-en">
              A complete list of all employees in the system.
            </span>
            <span className="lang-th">รายชื่อพนักงานทั้งหมดในระบบ</span>
          </CardDescription>
        </CardHeader>
        <CardContent>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4 items-end">
              ...
              <div className="space-y-2">
                ...
                >
                  ...
                  <span className="lang-th">แผนก:</span>
                </Label>
                <Input
              ...
              <div className="space-y-2">
                ...
                <Input
                  id="filter-status"
                  placeholder="Active / Inactive / Left"
                  className="bg-white dark:bg-gray-900"
              ...
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead>
                    ...
                    <span className="lang-th">ตำแหน่ง</span>
                  </TableHead>
                  <TableHead className="text-right">
                    <span className="lang-en">Salary</span>
                    <span className="lang-th">เงินเดือน</span>
                  </TableHead>
                  <TableHead>
                    <span className="lang-en">Status</span>
                    <span className="lang-th">สถานะ</span>
              ...
              <TableBody>
                {isLoading ? (
                    ...
                    <TableRow key={record.employee_id}>
                      <TableCell>
                        <div className="flex items-center gap-3">
                          <Avatar>
                            <AvatarImage
                              src={record.documents.photo as string}
                              alt={record.personal_info.name}
                            />
                            <AvatarFallback>
                              {record.personal_info.name.charAt(0)}
                            </AvatarFallback>
                          </Avatar>
                          <div>
                      ...
                      <TableCell className="text-right font-mono">
                        ฿{record.job_details.salary.toLocaleString()}
                      </TableCell>
                      <TableCell>
                        <Badge
                          variant="outline"
                          className={cn(
                            record.job_details.status === "Active"
                              ? "text-green-700 border-green-300 bg-green-50"
                              : record.job_details.status === "Inactive"
                              ? "text-yellow-700 border-yellow-300 bg-yellow-50"
                              : record.job_details.status === "Left"
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-center space-x-2">
                        ...
                        <Button
                          ...
                          onClick={() => handleDeleteClick(record.employee_id)}
                        >
                  ...
                  <TableRow>
                    ...
                    >
                      <span className="lang-en">
                        No employees found. Click "Add Employee" to create a new
                        record.
                      </span>
                      <span className="lang-th">
                        ไม่พบพนักงาน คลิก "เพิ่มพนักงาน" เพื่อสร้างบันทึกใหม่
                      </span>
                    </TableCell>
                  </TableRow>
                )}
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              <span className="lang-en">
                This will mark the employee's status as "Left". The record will
                not be permanently deleted.
              </span>
              <span className="lang-th">
                การดำเนินการนี้จะเปลี่ยนสถานะพนักงานเป็น "ออก"
                บันทึกจะไม่ถูกลบอย่างถาวร
              </span>
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            ...
            >
              <span className="lang-en">Mark as Left</span>
              <span className="lang-th">ทำเครื่องหมายว่าออก</span>
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
