...
async function getDashboardData() {
    ...
    const upcomingMaintenance = allVehicles.flatMap(v => v.maintenance_history || [])
        .filter(m => {
    ...
    const totalRevenue = salesThisMonth.reduce((acc, v) => acc + (v.sale_details?.sale_details.sale_price || 0), 0);
    const totalSales = salesThisMonth.length;
    
    const recentSales = allVehicles
        .filter(v => v.status === 'Sold')
        .sort((a, b) => new Date(b.sale_details.sale_details.sale_date).getTime() - new Date(a.sale_details.sale_details.sale_date).getTime())
        .slice(0, 5);
      ...
      <div className="mt-8 grid gap-4 md:gap-8 lg:grid-cols-2 xl:grid-cols-3">
        <Card className="xl:col-span-2">
          <CardHeader>
            ...
            <CardDescription>
              <span className="lang-en">Monthly sales performance for the last 6 months.</span>
              <span className="lang-th">ผลการขายรายเดือนสำหรับ 6 เดือนที่ผ่านมา</span>
            </CardDescription>
          </CardHeader>
          <CardContent>
            <SalesOverviewChart />
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              <span className="lang-en">A list of the most recent vehicle sales.</span>
              ...
              <TableBody>
                {recentSales.length > 0 ? recentSales.map(sale => (
                  <TableRow key={sale.id}>
                    <TableCell>
                      <div className="font-medium">{sale.sale_details.buyer.name}</div>
                      <div className="hidden text-sm text-muted-foreground md:inline">
                        {sale.sale_details.buyer.contact}
                      </div>
                    </TableCell>
                    <TableCell>{sale.vehicle}</TableCell>
                    <TableCell className="text-right">฿{sale.sale_details.sale_details.sale_price.toLocaleString()}</TableCell>
                  </TableRow>
                )) : (
                  <TableRow>
