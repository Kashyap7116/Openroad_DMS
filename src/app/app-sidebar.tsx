...
const Logo = () => (
    <svg width="180" height="32" viewBox="0 0 180 32" fill="none" xmlns="http://www.w3.org/2000/svg" className="text-foreground dark:text-primary-foreground">
     <defs>
    ...
    <text x="40" y="22" fontFamily="serif" fontSize="18" fill="url(#sidebar-grad)" fontWeight="600" className="tracking-wider">
      OPENROAD
    </text>
  </svg>
);
      ...
      <SidebarHeader>
        <Link href="/dashboard" className="flex items-center gap-2">
           <div className="bg-background dark:bg-sidebar-background rounded-lg p-2 flex items-center justify-center">
             {state === 'expanded' ? <Logo /> :  <Car className="h-6 w-6 text-primary" />}
           </div>
       ...
       <SidebarFooter className="border-t">
         <div className="flex items-center gap-3 p-2">
           <Avatar className="h-10 w-10">
              <AvatarImage src="https://picsum.photos/100" alt="@shadcn" data-ai-hint="profile picture"/>
              <AvatarFallback>JD</AvatarFallback>
            </Avatar>
            <div className={cn("flex flex-col", state === "collapsed" && "hidden")}>
                <span className="font-semibold text-sm">John Doe</span>
                <span className="text-xs text-muted-foreground">john.doe@openroad.com</span>
            </div>
         </div>
      </SidebarFooter>
    </>
