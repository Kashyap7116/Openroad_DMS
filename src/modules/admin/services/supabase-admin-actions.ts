...
import type { UserProfile } from "../../auth/services/supabase-auth-actions";
  ...
  try {
    ...
    if (updates.email || updates.password) {
      ...
      const { error: authError } =
        await supabaseService.auth.admin.updateUserById(
...
export async function deleteSupabaseUser(
  ...
  try {
    ...
    const { error: authError } = await supabaseService.auth.admin.deleteUser(
      existingProfile.user_id
    );
 ...
 */
export async function getSupabaseUsers(): Promise<{
  success: boolean;
  users?: UserProfile[];
  error?: string;
}> {
 ...
 */
export async function getSupabaseAuthUsers(): Promise<{
  success: boolean;
  users?: any[];
  error?: string;
}> {
