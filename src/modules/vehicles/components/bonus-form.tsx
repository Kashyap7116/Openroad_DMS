...
export function BonusForm({
  ...
  const vehicleOptions = useMemo(() => {
    return [
      ...
      ...vehicles.map((v) => ({
        value: v.id,
        label: `${v.vehicle || "Vehicle Data Missing"} (${v.license_plate})`,
      })),
    ];
  }, [vehicles]);
  ...
  useEffect(() => {
    if (initialData) {
      ...
      const visitTypeMatch = Object.keys(visitTypes).find((vt) =>
        remarks.includes(vt)
      );
      const licensePlate = (initialData as any).license_plate;
      const manualVehicleMatch =
        licensePlate === "No Booking" ? remarks.match(/for (.*?)\./)?.[1] : "";
  ...
  useEffect(() => {
    if (formData.visit_type) {
      const amount =
        visitTypes[formData.visit_type as keyof typeof visitTypes] || 0;
        ...
        <div className="space-y-4 pt-4 pb-6">
          ...
          <>
            ...
            {formData.license_plate === "No Booking" && (
              <div className="space-y-2">
                <Label htmlFor="manualVehicleNumber">
            ...
            <div className="space-y-2">
              ...
              <Select
                onValueChange={(value) =>
                  handleSelectChange("visit_type", value)
                }
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="space-y-2">
              <Label htmlFor="amount">Total Bonus Amount (฿)</Label>
              <Input
                id="amount"
                type="text"
