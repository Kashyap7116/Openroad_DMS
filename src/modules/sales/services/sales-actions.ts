...
import {
  getVehicle,
  saveVehicle,
} from "../../vehicles/services/vehicle-actions";
...
export async function saveSalesCommission(commissionData: {
  ...
  try {
    ...
    for (const employeeId of employee_ids) {
      const payrollRecord = (await getPayrollDataForMonth(
        payrollYear,
        payrollMonth,
        employeeId
      )) || {
    ...
    return await handleAndLogApiError(
      error,
      `saveSalesCommission for vehicle ${vehicle_id}`
    );
  }
}
 ...
 * @returns An object indicating success or failure.
 */
export async function deleteSalesCommission(
  ...
  try {
    ...
    const commissionTx = vehicle.financial_history?.find(
      (tx: any) =>
        tx.transaction_id === transactionId &&
        tx.category === "Sales Commission"
    );
    if (!commissionTx) {
      return {
        success: false,
        error: "Commission transaction not found on vehicle.",
      };
    }

    const employeeIdsMatch =
      commissionTx.remarks.match(/for (.*?)\./)?.[1].split(", ") || [];
    ...
    vehicle.financial_history = (vehicle.financial_history || []).filter(
    ...
    for (const employeeId of employeeIds) {
      ...
      if (payrollRecord && payrollRecord.employee_financial_history) {
        payrollRecord.employee_financial_history =
          payrollRecord.employee_financial_history.filter(
            (adj: any) =>
              !adj.remarks.includes(`Sales commission for ${vehicle.vehicle}`)
          );
        await savePayrollData(payrollYear, payrollMonth, payrollRecord);
      }
    }
    ...
    return await handleAndLogApiError(
      error,
      `deleteSalesCommission for tx ${transactionId}`
    );
  }
}
