...
import { saveErrorLog } from "../services/alerts-actions";
...
export async function handleAndLogApiError(error: any, context: string) {
  ...
  try {
    ...
    console.error(
      "CRITICAL: Failed to analyze or log the original error.",
      analysisError
    );
    // Fallback error message if the analysis itself fails
    return {
      success: false,
      error: `An unexpected error occurred, and the AI error analysis also failed. Original context: ${context}. Original error: ${
